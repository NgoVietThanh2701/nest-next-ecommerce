...
export class ProductsService {
  ...
  async update(id: string, updateProductDto: UpdateProductDto): Promise<ProductResponseDto> {
    ...
    if (updateProductDto.sku && updateProductDto.sku !== existingProduct.sku) {
      ...
      if (skuTaken) {
        throw new ConflictException(`A product with SKU '${updateProductDto.sku}' already exists.`);
      }
    }
  ...
  async updateStock(id: string, quantity: number): Promise<ProductResponseDto> {
    ...
    if (newStock < 0) {
      throw new BadRequestException(
        `Insufficient stock. Current stock: ${existingProduct.stock}, attempted adjustment: ${quantity}.`,
      );
    }
  ...
  async remove(id: string): Promise<{ message: string }> {
    ...
    if (existingProduct.orderItems.length > 0) {
      throw new BadRequestException(
        'Cannot delete product that has been ordered. Please mark it as inactive instead.',
      );
    }
