// DTO for category response

import { ApiProperty } from '@nestjs/swagger';

export class CategoryResponseDto {
  @ApiProperty({ example: '55-32egc', description: 'The unique identifier of the category' })
  id!: string;

  @ApiProperty({ example: 'Electronics', description: 'The name of the category' })
  name!: string;

  @ApiProperty({
    example: 'All kinds of electronic devices and gadgets',
    description: 'A brief description of the category',
    nullable: true,
  })
  description!: string | null;

  @ApiProperty({
    example: 'electronics',
    description: 'the URL-friendly slug for the category',
    nullable: true,
  })
  slug!: string | null;

  @ApiProperty({
    example: 'https://example.com/images/electronics.jpg',
    description: 'URL of the category image',
    nullable: true,
  })
  imageUrl!: string | null; // Optional field for category image URL

  @ApiProperty({
    example: true,
    description: 'Indicates if the category is active',
  })
  isActive: boolean; // Indicates if the category is active

  @ApiProperty({ example: 42, description: 'Number of products in this category' })
  productCount!: number; // Number of products in this category

  @ApiProperty({
    example: '2024-06-01T12:00:00Z',
    description: 'Timestamp of when the category was created',
  })
  createdAt!: Date; // Timestamp of when the category was created

  @ApiProperty({
    example: '2024-06-10T15:30:00Z',
    description: 'Timestamp of when the category was last updated',
  })
  updatedAt!: Date; // Timestamp of when the category was last updated
}
